...
export function UsersComponent({ contentHeight }: Props) {
  ...
  const loadUsers = async (isInitialLoad = false, query?: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t('users:failedToLoadUsers');
  ...
  const handleActivationToggle = async (userId: string, isActive: boolean) => {
    ...
    try {
      if (isActive) {
        ...
        message.success(t('users:userDeactivatedSuccessfully'));
      } else {
        await userManagementApi.activateUser(userId);
        message.success(t('users:userActivatedSuccessfully'));
      }
    } catch (error: unknown) {
  ...
  const handleRoleChange = async (userId: string, newRole: UserRole) => {
    ...
    try {
      ...
      message.success(t('users:userRoleChangedSuccessfully'));
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : t('users:failedToChangeUserRole');
      message.error(errorMessage);
    ...
    },
    {
      ...
      render: (role: UserRole, record: UserProfile) => (
        <Select
          ...
          options={[
            {
              label: <span style={{ color: getRoleColor(UserRole.ADMIN) }}>{t('users:admin')}</span>,
              value: UserRole.ADMIN,
            },
            {
              label: <span style={{ color: getRoleColor(UserRole.MEMBER) }}>{t('users:member')}</span>,
    ...
    return (
      ...
      >
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-600 dark:text-gray-400">{t('users:active')}:</span>
            <Switch
      ...
      <div className="w-full">
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <h1 className="text-2xl font-bold dark:text-white">{t('users:databasusUsers')}</h1>
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {isLoading ? t('common:loading') : t('users:usersCount', { current: users.length, total })}
            </div>
          </div>
