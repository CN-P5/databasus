...
const getRoleDisplayText = (role: UserRole, t: (key: string) => string): string => {
  switch (role) {
    case UserRole.ADMIN:
...
export function ProfileComponent({ contentHeight }: Props) {
  ...
  const validatePasswordFields = (): boolean => {
    ...
    if (!newPassword) {
      ...
      setNewPasswordError(true);
      message.error(t('users:passwordMinLength'));
      isValid = false;
    } else {
    ...
    if (!confirmPassword) {
      ...
      setConfirmPasswordError(true);
      message.error(t('users:passwordsDoNotMatch'));
      isValid = false;
    } else {
    ...
    try {
      ...
      if (user?.email) {
        try {
          ...
          message.success(t('users:signedInWithNewPassword'));
        } catch (signInError: unknown) {
          const errorMessage =
            signInError instanceof Error
              ? signInError.message
              : t('users:failedToSignInWithNewPassword');
      ...
      const errorMessage = error instanceof Error ? error.message : t('users:failedToChangePassword');
      message.error(errorMessage);
    } finally {
  ...
  const handleProfileUpdate = async () => {
    ...
    if (!editName || editName.trim() === '') {
      ...
      message.error(t('users:nameRequired'));
      return;
    }
    ...
    try {
      ...
      if (Object.keys(request).length === 0) {
        message.info(t('users:noChangesToSave'));
      ...
      const errorMessage = error instanceof Error ? error.message : t('users:failedToUpdateProfile');
      message.error(errorMessage);
    } finally {
      setIsUpdatingProfile(false);
    }
  ...
  return (
    <div className="flex grow">
      <div className="w-full">
        ...
        >
          <h1 className="text-2xl font-bold dark:text-white">{t('users:profile')}</h1>
              ...
              <>
                <div className="mb-6">
                  ...
                  <div className="max-w-md">
                    <div className="text-xs font-semibold dark:text-gray-200">{t('users:userId')}</div>
                    <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">{user.id}</div>

                    <div className="mb-1 text-xs font-semibold dark:text-gray-200">{t('users:name')}</div>
                    <Input
                      value={editName}
                      onChange={(e) => {
                        setEditNameError(false);
                    ...
                    <div className="mt-2 mb-1 text-xs font-semibold dark:text-gray-200">{t('users:email')}</div>
                    <Input
                      value={editEmail}
                      onChange={(e) => {
                        setEditEmailError(false);
                    ...
                    {user.email === 'admin' && (
                      <div className="mb-4 text-xs text-gray-500 dark:text-gray-400">
                        {t('users:adminEmailCannotBeChanged')}
                      </div>
                    )}
                    ...
                    <div className="mb-4">
                      <span className="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                        {getRoleDisplayText(user.role, t)}
                      </span>
                    </div>
                ...
                <div className="max-w-xs">
                  <h3 className="mb-4 text-lg font-semibold dark:text-white">{t('users:changePassword')}</h3>
                  ...
                  <div className="max-w-sm">
                    ...
                    {(newPassword || confirmPassword) && (
                      <Button
                        ...
                        className="border-blue-600 bg-blue-600 hover:border-blue-700 hover:bg-blue-700"
                      >
                        {isChangingPassword ? t('users:changingPassword') : t('users:changePassword')}
                      </Button>
                    )}
                  </div>
                </div>
