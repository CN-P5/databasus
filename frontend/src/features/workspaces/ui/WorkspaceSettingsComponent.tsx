...
export function WorkspaceSettingsComponent({ workspaceResponse, user, contentHeight }: Props) {
  ...
  const loadWorkspace = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t('workspaces:failedToLoadWorkspace');
  ...
  const handleDeleteWorkspace = async () => {
    ...
    modal.confirm({
      ...
      content: (
        <div>
          <p>
            {t('workspaces:deleteWorkspaceConfirmation', { name: workspace.name })}
          </p>
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (databases && databases.length > 0) {
            message.error(
              t('workspaces:cannotDeleteWorkspacePleaseRemoveAllDatabasesFirst', { count: databases.length }),
            );
            return;
          }
  ...
  return (
    <div className="flex grow">
      <div className="w-full">
        ...
        >
          <h1 className="mb-6 text-2xl font-bold dark:text-white">{t('workspaces:workspaceSettings')}</h1>
