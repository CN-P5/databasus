...
func Test_IsUserReadOnly_AdminUser_ReturnsFalse(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			isReadOnly, privileges, err := pgModel.IsUserReadOnly(ctx, logger, nil, uuid.New(), nil)
			assert.NoError(t, err)
...
func Test_IsUserReadOnly_ReadOnlyUser_ReturnsTrue(t *testing.T) {
	...
	username, password, err := pgModel.CreateReadOnlyUser(ctx, logger, nil, uuid.New(), nil)
	assert.NoError(t, err)
...
func Test_CreateReadOnlyUser_Supabase_UserCanReadButNotWrite(t *testing.T) {
	...
	connectionUsername, newPassword, err := pgModel.CreateReadOnlyUser(ctx, logger, nil, uuid.New(), nil)
	assert.NoError(t, err)
